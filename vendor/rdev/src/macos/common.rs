...
pub unsafe fn convert(
    ...
    if let Some(event_type) = option_type {
        // NOTE: simple-ptt does not consume Event.name for global hotkeys.
        // On modern macOS versions, deriving a printable character for every
        // keypress here can crash inside Text Input Services with
        // _dispatch_assert_queue_fail / TISGetInputSourceProperty when the app
        // is running alongside AppKit text input.
        //
        // Upstream rdev computes Event.name for keypresses by calling into TIS.
        // For this app's grab-based hotkey handling, that extra work is both
        // unnecessary and actively unsafe, so we omit it on macOS.
        let name = None;
        return Some(Event {
            event_type,
            time: SystemTime::now(),
